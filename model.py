...
class Model:
	...
	def __init__(self, img_size):
		...
		xavier_init = 1.0/(config.filter_size*config.filter_size*config.input_channels)
		self.filter1 = tf.Variable(tf.truncated_normal([config.filter_size, config.filter_size, 
			config.input_channels, config.conv1_channels], mean=0.0, stddev=xavier_init), name='convlayer1')
		...
		self.filter2 = tf.Variable(tf.truncated_normal([config.filter_size, config.filter_size, 
			config.conv1_channels, config.output_channels], mean=0.0, stddev=xavier_init), name='convlayer2')
		...
		self.weights = tf.Variable(tf.random_normal((fan_in, 1), 
			stddev=1.0/fan_in, dtype=tf.float32), name="weights", trainable=True)
